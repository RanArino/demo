...
func (c *UserClient) QueryPreferences(u *User) *UserPreferencesQuery {
	query := (&UserPreferencesClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(userpreferences.Table, userpreferences.FieldID),
...
// UpdateOne returns an update builder for the given entity.
func (c *UserPreferencesClient) UpdateOne(up *UserPreferences) *UserPreferencesUpdateOne {
	mutation := newUserPreferencesMutation(c.config, OpUpdateOne, withUserPreferences(up))
	return &UserPreferencesUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *UserPreferencesClient) UpdateOneID(id uuid.UUID) *UserPreferencesUpdateOne {
...
// DeleteOne returns a builder for deleting the given entity.
func (c *UserPreferencesClient) DeleteOne(up *UserPreferences) *UserPreferencesDeleteOne {
	return c.DeleteOneID(up.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *UserPreferencesClient) DeleteOneID(id uuid.UUID) *UserPreferencesDeleteOne {
...
func (c *UserPreferencesClient) Get(ctx context.Context, id uuid.UUID) (*UserPreferences, error) {
	return c.Query().Where(userpreferences.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *UserPreferencesClient) GetX(ctx context.Context, id uuid.UUID) *UserPreferences {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryUser queries the user edge of a UserPreferences.
func (c *UserPreferencesClient) QueryUser(up *UserPreferences) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := up.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(userpreferences.Table, userpreferences.FieldID, id),
...
func (c *UserPreferencesClient) mutate(ctx context.Context, m *UserPreferencesMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("ent: unknown UserPreferences mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		User, UserPreferences []ent.Hook
	}
	inters struct {
		User, UserPreferences []ent.Interceptor
	}
)
