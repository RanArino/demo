...
type UserQuery struct {
	...
	withPreferences *UserPreferencesQuery
...
func (uq *UserQuery) QueryPreferences() *UserPreferencesQuery {
	query := (&UserPreferencesClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithPreferences(opts ...func(*UserPreferencesQuery)) *UserQuery {
	query := (&UserPreferencesClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPreferences = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPreferences; query != nil {
		if err := uq.loadPreferences(ctx, query, nodes, nil,
			func(n *User, e *UserPreferences) { n.Edges.Preferences = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPreferences(ctx context.Context, query *UserPreferencesQuery, nodes []*User, init func(*User), assign func(*User, *UserPreferences)) error {
	...
	query.withFKs = true
	query.Where(predicate.UserPreferences(func(s *sql.Selector) {
	...
	for _, n := range neighbors {
		fk := n.user_preferences
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_preferences" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_preferences" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
