...
// Generate mock documents (Layer 2 - z=200)
const generateDocumentNodes = (): MockNode[] => {
  ...
  return documents.map((doc, index) => ({
    ...
    depth_level: 2,
    path_to_root: `/${doc.id}`,
    position_2d: { x: Math.cos(index * 2 * Math.PI / 5) * 150, y: Math.sin(index * 2 * Math.PI / 5) * 150 },
    position_3d: { x: Math.cos(index * 2 * Math.PI / 5) * 150, y: Math.sin(index * 2 * Math.PI / 5) * 150, z: 200 },
...
const generateClusterNodes = (): MockNode[] => {
  ...
  return clusters.map((cluster, index) => ({
    ...
    position_2d: { x: Math.cos(index * 2 * Math.PI / 6) * 100, y: Math.sin(index * 2 * Math.PI / 6) * 100 },
    position_3d: { x: Math.cos(index * 2 * Math.PI / 6) * 100, y: Math.sin(index * 2 * Math.PI / 6) * 100, z: 0 },
    is_position_locked: false,
    visibility: true,
    display_props: {
      size: 12,
...
// Generate mock text chunks (Layer 0 - z=-200)
const generateChunkNodes = (): MockNode[] => {
  ...
  chunkTopics.forEach((topic, index) => {
    const angle = (index * 2 * Math.PI) / chunkTopics.length;
    ...
    chunks.push({
      ...
      position_2d: { x: Math.cos(angle) * radius, y: Math.sin(angle) * radius },
      position_3d: { x: Math.cos(angle) * radius, y: Math.sin(angle) * radius, z: -200 },
      is_position_locked: false,
      visibility: true,
      display_props: {
        size: 8,
