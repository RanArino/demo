...
export default function SpaceCanvas({ space, className }: SpaceCanvasProps) {
  ...
  const { nodes, isLoading, errorMessage, reload: reloadNodes } = useCanvasData(space.id);
  ...
  const selectionDetails = selectedNode
    ? [
        ...
        selectedNode.title ? { label: 'Title', value: selectedNode.title } : null,
        selectedNode.displayContent ? { label: 'Display', value: selectedNode.displayContent } : null,
      ].filter(Boolean)
    : [];
  ...
  return (
    <div className={cn('flex h-full flex-col bg-slate-950 text-slate-100', className)}>
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        <div
          className={cn(
            'pointer-events-none absolute bottom-4 left-4 h-36 w-48 overflow-hidden rounded-lg border border-slate-700 bg-slate-900/80 backdrop-blur transition-opacity',
        ...
        <div
          className={cn(
            'absolute top-4 right-4 z-20 w-52 transition-opacity',
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {multiViewPresets.map(({ key, label, ref }) => (
              <button
                ...
                className="group relative h-28 w-full overflow-hidden rounded-lg border border-slate-700 bg-slate-900/80 backdrop-blur transition ring-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
              >
